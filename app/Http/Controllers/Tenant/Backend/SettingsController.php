...
//get settings data from axios
    ...
    public function editAllSettings(){
        ...
        return Inertia::render('Tenant/Website/Backend/Settings/EditSettings',
            [
                'settings' => Settings::all()
            ]);
    }
    ...
    public function updateAllSettings(Request $request, $id ){
        ...
        if ($request->file('logo')) {
            $file = $request->file('logo');
            @unlink(public_path('upload/tenant/backend/settings/'.$data->logo));
            $filename = date('YmdHi').$file->getClientOriginalName();
            $file->move(public_path('upload/tenant/backend/settings'),$filename);
            $data['logo'] = $filename;
        }
        ...
        if ($request->file('footer_logo')) {
            $file = $request->file('footer_logo');
            @unlink(public_path('upload/tenant/backend/settings/'.$data->footer_logo));
            $filename = date('YmdHi').$file->getClientOriginalName();
            $file->move(public_path('upload/tenant/backend/settings'),$filename);
            $data['footer_logo'] = $filename;
        }
        ...
        if ($request->file('image')) {
            $file = $request->file('image');
            @unlink(public_path('upload/tenant/backend/settings/'.$data->image));
            $filename = date('YmdHi').$file->getClientOriginalName();
            $file->move(public_path('upload/tenant/backend/settings'),$filename);
            $data['image'] = $filename;
        }
